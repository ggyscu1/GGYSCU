...
void MainController::draw_japanese_temple() {
...
void MainController::update() {
    ...
    if (!m_event_started && platform->key(engine::platform::KeyId::KEY_E).is_down()) {
    ...
    if (m_event_started) {
        ...
        if (!m_event_a_done && m_event_timer >= 2.0f) {
            m_event_a_done = true;
        }
        if (!m_event_b_done && m_event_timer >= 5.0f) {
            m_event_b_done = true;
        }
    }
}
